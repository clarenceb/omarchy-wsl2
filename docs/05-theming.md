...
Use this project's helper rather than Omarchy's:

```bash
omarchy-wsl-bg                 # list what's available, * marks the current one
omarchy-wsl-bg 3               # set the third
omarchy-wsl-bg next            # cycle
omarchy-wsl-bg ~/pic.png       # set a specific image
omarchy-wsl-bg --colour 0d1120 # flat colour instead
```

It applies the change to the **running** session immediately.

> **Why not `omarchy-theme-bg-next`?** That updates the `current/background`
> symlink and then pushes the change over IPC to `omarchy-shell` (Quickshell),
> which is Hyprland-side and isn't running here — so nothing happens on
> screen. `swaybg` also reads its image once at startup and must be restarted.
> `omarchy-wsl-bg` updates the same symlink, so Omarchy's own tooling stays in
> step, then restarts `swaybg` against the live sway socket.

### Where wallpapers come from

| Source | Path |
|---|---|
| The active theme | `current/theme/backgrounds/` |
| Your own, per theme | `~/.config/omarchy/backgrounds/<theme>/` |
| Your own, anywhere | `~/Pictures/wallpapers/` |
| Shipped with this project | `/usr/share/omarchy-wsl2/wallpapers/` |

The shipped set is four dark gradients generated by `scripts/make-wallpaper.py`
(`omarchy-wsl2`, `tokyo-night`, `ember`, `matte-black`). They exist so a fresh
image has a real background before any theme is applied. Regenerate or add your
own palettes by editing that script and running `make logo`.

> Omarchy moved its state from `~/.config/omarchy/current` to
> `~/.local/state/omarchy/current`; `omarchy-wsl-bg` handles both.

In headless mode there's nothing to draw a wallpaper on, which is why the build
sets `OMARCHY_THEME_SKIP_BACKGROUND=1`.
